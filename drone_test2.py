...
class DroneEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self._in_bounds(new_position) and not self._in_no_fly_zone(new_position):
            ...
            self.velocity = 0.0
        done = 9.0 <= self.position[0] <= 10.0 and 9.0 <= self.position[1] <= 10.0
    ...
    def render(self, mode="human"):
        ...
        plt.fill([9, 10, 10, 9], [9, 9, 10, 10], color='green', alpha=0.3, label="Goal Area")
...
model = PPO.load("drone_ppo_model") 
